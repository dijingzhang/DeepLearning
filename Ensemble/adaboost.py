...
class AdaBoost(object):
    def __init__(self, M, clf, learning_rate=1.0, method="discrete", tol=None, weight_trimming=None):
        """
        :param M: epochs
        :param clf: classifier (base learner)
        :param learning_rate: learning rate for adaboost
        :param method: discrete (-1/1) or real (probability 0-1)
        :param tol: the threshold for early stopping. Acc on test below tol -> stop training
        :param weight_trimming: delete the low-weighted samples. Set a threshold like 90% to abandon.
        """
        self.M = M
        self.clf = clf
    ...
    def fit(self, X, y):
        ...
        w = np.array([1 / len(X)] * len(X))  # initialization wi = 1 / N (i = 1, 2,..., N)
        self.clf_total = []
        self.alpha_total = []
        ...
        for m in range(self.M):
            ...
            if self.method == "discrete":
                if m >= 1 and self.weight_trimming is not None:
                    # the implementation of weight trimming
                    # sort the weights -> compute cumulative sum -> abandon the low-weight samples
                    sort_w = np.sort(w)[::-1]
                ...
                loss = np.zeros(len(X))
                loss[y_pred != y] = 1
                err = np.sum(w * loss)  # compute the error rate
                alpha = 0.5 * np.log((1 - err) / err) * self.learning_rate  # compute alpha_m, coefficient of base learner
                w = (w * np.exp(-y * alpha * y_pred)) / np.sum(w * np.exp(-y * alpha * y_pred))  # update wi
