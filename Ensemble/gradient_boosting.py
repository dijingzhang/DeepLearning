...
# Regress: squared loss, huber loss
# Classification: logistic loss, modified huber loss
...
class GradientBoosting(object):
    ...
    def fit(self, X, y):
        ...
        y_pred = init_learner.fit(X, y).predict(X)  # initialization
    ...
    def predict(self, X):
        ...
        pred = np.vstack((self.base_learner_total[0].predict(X), pred))  # initialization + base learners
        if self.method == "regression":
